...
export default function ValidationEngine({
  ...
  const getDefaultErrorMessage = (field: string, ruleType: string, ruleValue: any): string => {
    switch (ruleType) {
      case 'required':
  ...
  const getDangerousFields = (): string[] => {
    return fieldConfigs.filter(field => field.is_dangerous).map(field => field.field_name)
  }
  ...
  const runValidation = () => {
    ...
    const dangerousChanges = Object.entries(testValues).filter(([fieldName, value]) => {
      return getDangerousFields().includes(fieldName) && value && value.toString().trim() !== ''
    })
        ...
        await supabase.from('audit_logs').insert({
          ...
          details: {
            ...
            dangerous_fields: getDangerousFields().filter(
              field => pendingChanges[field] && pendingChanges[field].toString().trim() !== ''
            ),
          },
        })
