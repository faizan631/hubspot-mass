...
export default function GoogleConnection({
  ...
  const fetchSheets = async () => {
    ...
    try {
      ...
      if (data.sheets) {
        ...
        if (data.sheets.length === 0) {
          toast({
            ...
            description: 'No Google Sheets found in your account. Create a sheet first.',
            variant: 'destructive',
          })
        }
      } else if (data.error) {
        ...
        toast({
          title: 'Error',
          description: 'Failed to fetch Google Sheets. Please try reconnecting.',
          variant: 'destructive',
        })
      }
    } catch (error) {
      ...
      toast({
        title: 'Error',
        description: 'Failed to fetch Google Sheets. Please check your connection.',
        variant: 'destructive',
      })
    }
  ...
  const handleSheetSelect = (sheetId: string) => {
    ...
    const selectedSheetName = sheets.find(sheet => sheet.id === sheetId)?.name
    toast({
    ...
    return (
      <Card>
        <CardHeader>
          ...
          <CardDescription>Connect your Google account to access sheets</CardDescription>
        </CardHeader>
        <CardContent>
        ...
        {!isConnected ? (
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              <label className="text-sm font-medium">Select a Google Sheet to sync with:</label>
              {fetchingSheets ? (
