...
export default function SyncManager({ user, sheetId, sheetName }: SyncManagerProps) {
  ...
  const previewChanges = async () => {
    if (!sheetId || !sheetName) {
        toast({ title: "Configuration Missing", description: "Sheet ID and Name are required.", variant: "destructive"});
        return;
    }
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          After editing content in Google Sheets, preview the changes here before syncing them back to HubSpot.
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-6">
        <Button onClick={previewChanges} disabled={isPreviewing || !canPreview} className="w-full" size="lg">
          {isPreviewing ? (
            <>
            ...
            {changes.map((change) => (
              <div key={change.pageId} className="border p-4 rounded-lg space-y-3">
                <h4 className="font-semibold text-base">
                  {change.name} <span className="text-xs font-mono text-gray-500">(ID: {change.pageId})</span>
                </h4>
                ...
                {change.fields.name && (
                  <div>
                    ...
                    <div className="text-sm p-2 rounded bg-gray-50">
                        ...
                        <ins className="text-green-600 no-underline ml-2">{change.fields.name.new}</ins>
                    </div>
                  </div>
                )}
        ...
        {!isPreviewing && changes.length === 0 && (
             <p className="text-sm text-center text-gray-500 py-4">Click "Preview Changes" to check for modifications.</p>
        )}
      </CardContent>
    </Card>
  );
