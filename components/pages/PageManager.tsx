...
import { Globe, Search, RefreshCw, ExternalLink, Calendar, FileText, AlertCircle } from "lucide-react"
...
export default function PageManager({ user, userSettings }: PageManagerProps) {
  ...
  const loadPages = async () => {
    ...
    try {
      const endpoint = isPaidConnection ? "/api/hubspot/pages" : "/api/hubspot/free-tier-pages"
    ...
    <div className="space-y-6">
        ...
        <CardContent className="pt-6">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <div>
                <h3 className="font-medium">{isPaidConnection ? "Full CMS Access" : "Free Tier Access"}</h3>
                <p className="text-sm text-gray-600">
                  {isPaidConnection
                    ? "Access to all HubSpot CMS pages via API"
                    : "Website scraping for publicly available pages"}
                </p>
              </div>
            </div>
            <Badge variant={isPaidConnection ? "default" : "secondary"}>{isPaidConnection ? "Premium" : "Free"}</Badge>
          </div>
        </CardContent>
      </Card>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            {isPaidConnection ? "Manage your HubSpot CMS pages" : "View publicly available pages from your website"}
          </CardDescription>
        </CardHeader>
                ...
                <div className="flex items-start justify-between">
                  <div className="flex-1 space-y-2">
                    <div className="flex items-center gap-2">
                      ...
                      {page.state && (
                        <Badge variant={page.state === "PUBLISHED" ? "default" : "secondary"}>{page.state}</Badge>
                      )}
                    </div>

                    {page.htmlTitle && <p className="text-sm text-gray-600">{page.htmlTitle}</p>}

                    {page.metaDescription && (
                      <p className="text-xs text-gray-500 line-clamp-2">{page.metaDescription}</p>
                    )}
              ...
              {pages.length === 0 ? (
                <>
                  ...
                  <p className="text-gray-600 mb-4">
                    {isPaidConnection ? "No pages found in your HubSpot CMS" : "No pages found on your website"}
                  </p>
                ...
                <>
                  ...
                  <h3 className="text-lg font-medium text-gray-900 mb-2">No Matching Pages</h3>
