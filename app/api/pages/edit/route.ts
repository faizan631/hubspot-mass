...
export async function POST(request: NextRequest) {
  try {
    const { userId, pageId, updates, hubspotToken, sheetId } = await request.json()
...
async function updateSheetTab(
  ...
  try {
    ...
    if (error instanceof Error && error.message.includes('already exists')) {
      ...
      console.log(`Tab "${tabName}" already exists in sheet ${sheetId}`)
      return
    }
  ...
  // Add new data
  await fetch(`https://sheets.googleapis.com/v4/spreadsheets/${sheetId}/values/${tabName}!A1`, {
    method: 'PUT',
    headers: {
      Authorization: `Bearer ${accessToken}`,
      'Content-Type': 'application/json',
