...
export async function POST(request: NextRequest) {
  try {
    ...
    const pageTypeMap = new Map(
      pageTypesData.map(p => [p.hubspot_page_id, p.page_type])
    );
    ...
    for (const change of changes) {
        ...
        if (pageType === "Site Page") {
            ...
            failed.push({ pageId, name: change.name, error: `Syncing for page type '${pageType}' is not supported yet.` });
            continue;
        }
        ...
        try {
            ...
            if (response.ok) {
                ...
                failed.push({ pageId, name: change.name, error: errorData.message || `HTTP Error ${response.status}` });
            }

        } catch (error) {
            failed.push({ pageId, name: change.name, error: error instanceof Error ? error.message : "Network error" });
        }
    }
    ...
    return NextResponse.json({ success: false, error: `Failed to sync changes: ${errorMessage}` }, { status: 500 });
  }
}