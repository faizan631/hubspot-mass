...
export async function POST(request: NextRequest) {
  try {
    ...
    const { hubspot_token, hubspot_connection_type, website_domain, google_refresh_token, backup_sheet_id } = body
    ...
    if (existingSettings) {
      ...
      const { error: updateError } = await supabase.from("user_settings").update(updateData).eq("user_id", user.id)
