...
export async function POST(request: NextRequest) {
  ...
  try {
    const { domain, hubspotToken } = await request.json()
    ...
    for (let i = 0; i < discoveredPages.length; i++) {
      ...
      try {
        ...
        if (pageResponse.ok) {
          ...
          let bodyText = ""
          const contentSelectors = ["main", ".content", "#content", "article", ".post-content", "body"]
    ...
    return NextResponse.json({
      ...
      summary: {
        ...
        averageContentLength: Math.round(
          scrapedPages.reduce((sum, page) => sum + (page.content.bodyText?.length || 0), 0) / scrapedPages.length,
        ),
      },
    })
  } catch (error) {
