...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const endpoints = [
      ...
      { name: "Site Pages", url: "https://api.hubapi.com/cms/v3/site-pages?limit=100", key: "results" },
      { name: "Content Pages", url: "https://api.hubapi.com/content/api/v2/pages?limit=100", key: "objects" },
    ];
    ...
    const mappedPages = pages.map((page: any, i: number) => ({
      ...
      url: page.url || page.absolute_url || page.published_url || `https://dummy.com/${page.slug || ""}`,
    ...
    return NextResponse.json({ success: false, error: "Failed to fetch pages", details: error instanceof Error ? error.message : String(error) }, { status: 500 });
  }
}
